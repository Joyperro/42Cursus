...
/*   Updated: 2024/03/18 15:34:02 by dclement         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	tile_check(char **map, t_mapcheckdata *data)
{
	int	x;
	int	y;
...
int	is_valid_map(char **map)
{
	...
	if (data.size.y == data.size.x)
		valid = error("Map has to be rectangular");
	...
	if (data.v_player == FALSE || data.v_collectable == FALSE || data.v_exit == FALSE)
		valid = error("there has to be one 'P' and 'E' and at least one 'C'");
	return (valid);
}
